...
│   │   ├── models.py               \# ORM 模型定义（映射 app.db 表结构）  
...
│   └── start.sh                    \# 容器启动脚本（seed + uvicorn）  
│  
├── backend_data/                   \# SQLite 数据持久化目录（运行时生成）  
...
* 如需本地非 Docker 方式演示，请安装 **Python 3.9+**。
* 确保 **80** 端口（Docker）与 **8000/5500** 端口（本地演示）未被占用。

### **3.2 方案一：Docker 本地演示 (推荐)**

1. 在项目根目录打开终端，运行：  
   ```Bash
   docker compose up --build
   ```
2. 访问页面：http://localhost
3. 检查后端健康接口：http://localhost/api/health

说明：SQLite 数据会持久化到 `backend_data/` 目录。

### **3.3 方案二：本地非 Docker 演示 (保留)**

1. **配置 API 地址**：  
   打开 frontend/app.js，将 API 基地址改为本地后端地址：  
   ```JavaScript
   const API_BASE = "http://localhost:8000";
   ```
   （Docker 方案使用 `/api`，非 Docker 方案需改回本地地址。）
2. **安装依赖**：  
   在项目根目录打开终端，运行：  
   ```Bash  
...
可选：如果 80/8000/5500 端口被占用，可自行改用 8080/8001/5501 等端口，并同步修改 `API_BASE`。
...
### **4.2 方案一：Docker 部署 (推荐)**

1. **上传代码**：将项目目录上传到服务器（例如 /var/www/zju-nav）。
2. **安装 Docker**：安装 Docker Engine 与 Docker Compose。
3. **启动服务**：
...
4. **访问服务**：浏览器访问 `http://<服务器IP>/`。

说明：数据库文件持久化在 `backend_data/` 目录，建议定期备份。

### **4.3 方案二：传统部署 (Nginx + Uvicorn)**
